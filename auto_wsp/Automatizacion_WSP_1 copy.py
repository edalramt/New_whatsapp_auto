...
# Función para generar un nombre de archivo incremental
def generar_nombre_archivo(base_dir, base_name):
    """
    Genera un nombre de archivo incremental basado en los archivos existentes.
    Ejemplo: Contactos_enviados_1.xlsx, Contactos_enviados_2.xlsx, etc.
    """
    contador = 1
    while True:
        archivo = os.path.join(base_dir, f"{base_name}_{contador}.xlsx")
        if not os.path.exists(archivo):
            return archivo  # Retorna el primer nombre disponible
        contador += 1

# Función para obtener el archivo más reciente
def obtener_ultimo_archivo(base_dir, base_name):
    """
    Obtiene el archivo más reciente basado en el patrón Contactos_enviados_X.xlsx.
    """
    archivos = [f for f in os.listdir(base_dir) if f.startswith(base_name) and f.endswith(".xlsx")]
    archivos_validos = []

    for archivo in archivos:
        try:
            # Extraer el número del archivo y verificar que sea válido
            numero = int(archivo.split("_")[-1].split(".")[0])
            archivos_validos.append((numero, archivo))
        except ValueError:
            # Ignorar archivos que no sigan el formato esperado
            continue

    if not archivos_validos:
        return None  # No hay archivos válidos

    # Ordenar por número y retornar el archivo más reciente
    archivos_validos.sort(key=lambda x: x[0])
    return os.path.join(base_dir, archivos_validos[-1][1])  # Retornar el archivo más reciente

# Definir rutas de los archivos
archivo_preinscritos = os.path.join(logistics_dir, "Preinscritos.xlsx")
base_archivo_enviados = "Contactos_enviados"
...
archivo_enviados_reciente = obtener_ultimo_archivo(logistics_dir, base_archivo_enviados)

if archivo_enviados_reciente is None:
    print("No se encontró un archivo previo de enviados. Se generará uno nuevo.")
    archivo_enviados_reciente = generar_nombre_archivo(logistics_dir, base_archivo_enviados)
    verificar_o_crear_archivo(archivo_enviados_reciente, columnas)

# Cargar los números enviados desde el archivo más reciente
df_enviados_reciente = pd.read_excel(archivo_enviados_reciente)
enviados = df_enviados_reciente["Celular"].astype(str).str.replace(" ", "").str.strip().tolist()

# Generar un nuevo archivo incremental para el proceso actual
archivo_enviados_actual = generar_nombre_archivo(logistics_dir, base_archivo_enviados)

# Crear el nuevo archivo con las columnas predeterminadas
verificar_o_crear_archivo(archivo_enviados_actual, columnas)

# Crear un DataFrame vacío para el nuevo archivo
df_enviados_actual = pd.DataFrame(columns=columnas)
...
def main():
    global programa_activo, df_enviados_actual  # Declaración global

    # Load data from the 'archivo_preinscritos' file
    ...
    for _, row in df_preinscritos.iterrows():
        ...
        if estado != "Válido":
            ...
            df_enviados_actual = pd.concat([df_enviados_actual, pd.DataFrame([{
                "Nombre": nombre,
    ...
    if programa_activo:  # Solo guardar si el programa no fue detenido
        guardar_datos(df_enviados_actual, archivo_enviados_actual)
        print(f"Proceso finalizado. Los mensajes fueron enviados y registrados en {archivo_enviados_actual}.")
    else:
